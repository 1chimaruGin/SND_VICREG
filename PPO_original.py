...
class PPO:
    ...
    def train(self, memory, indices):
        if indices:
            ...
            if self._motivation:
                ...
                adv_values = (
                    ext_adv_values * self.ext_adv_scale
                    + int_adv_values * self.int_adv_scale
                )

            else:
                ref_values, adv_values = self.calc_advantage(
                    values, rewards, dones, self._gamma[0], self._n_env
                )
    ...
    def _train(self, states, actions, probs, adv_values, ref_values):
        ...
        for epoch in range(self._ppo_epochs):
            for batch_ofs in range(0, self._trajectory_size, self._batch_size):
                ...
                loss = self.calc_loss(
                    states_v, batch_ref_v, batch_adv_v, actions_v, probs_v
                )
                loss.backward()
    ...
    def calc_loss(self, states, ref_value, adv_value, old_actions, old_probs):
        ...
        loss = (
            loss_value * self._critic_loss_weight
            + loss_policy * self._actor_loss_weight
            + self._beta * entropy
        )
