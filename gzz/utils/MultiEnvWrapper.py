...
class MultiEnvParallel:
    def __init__(self, envs_list, envs_count, thread_count=4):
        ...
        self.observations = np.zeros(
            (envs_count,) + self.observation_space.shape, dtype=np.float32
        )
        # a = [None] * n_env
        self.rewards = np.zeros((envs_count, 1), dtype=np.float32)
        self.dones = np.zeros((envs_count, 1), dtype=np.float32)
